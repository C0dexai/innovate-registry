...
const AgentView: React.FC<AgentViewProps> = ({ agent }) => {
  ...
  const speak = useCallback((text: string, agent: Agent) => {
    if (!isVoiceSupported || voices.length === 0) return;
    ...
    if (agent.gender === 'Female') {
        selectedVoice = langVoices.find(v => v.name.includes('Female')) || langVoices.find(v => v.name.includes('Zira')) || langVoices.find(v => v.name.includes('Samantha'));
    } else {
        selectedVoice = langVoices.find(v => v.name.includes('Male')) || langVoices.find(v => v.name.includes('David')) || langVoices.find(v => v.name.includes('Tom'));
    }
  ...
  }, [chatSession, isChatLoading, agent, speak]);
    ...
    <div className="flex flex-col h-full bg-slate-900/50 rounded-lg">
      <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-slate-700/50">
        <div>
          ...
          <p className="text-sm text-red-400 font-light">{agent.role}</p>
        </div>
      </header>
        ...
        <form onSubmit={handleFormSubmit} className="flex items-center gap-3">
          ...
          {isVoiceSupported && (
            <button
                ...
                className={`p-2 rounded-full transition-colors ${isListening ? 'bg-red-600 animate-pulse' : 'bg-slate-700 hover:bg-slate-600'} disabled:bg-slate-800 disabled:cursor-not-allowed`}
