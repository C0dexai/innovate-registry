...
import { CrosshairIcon, FolderIcon } from './FileIcons';
...
const OrbMenu: React.FC<OrbMenuProps> = ({ activeView, setActiveView }) => {
    ...
    const orbRadius = 100; // in pixels
    const itemAngle = 360 / (menuItems.length * 2); // Adjust spacing
        ...
        <div className="fixed bottom-8 right-8 z-50">
            <div className="relative flex items-center justify-center w-20 h-20">
                ...
                {menuItems.map((item, index) => {
                    const angle = -90 - (itemAngle * (index - 1.5) * 2); // Start from top and go clockwise
