...
import { CrosshairIcon, OrchestratorIcon } from './components/FileIcons';

const App: React.FC = () => {
  const [activeView, setActiveView] = useState<ViewType>('orchestrator');
  ...
  const tabs = [
    ...
    { key: 'orchestrator', label: 'Orchestrator', isAgent: false, icon: <OrchestratorIcon className="h-4 w-4" /> },
          ...
          <nav className="-mb-px flex space-x-6 overflow-x-auto" aria-label="Tabs">
            {tabs.map((tab) => (
              <button
                ...
                className={`
                  ${activeView === tab.key
                    ? 'border-fuchsia-500 text-white' + ((tab.isAgent && tab.key === 'Kara') || ['workflow', 'orchestrator'].includes(tab.key) ? ' neon-glow-fuchsia' : '')
